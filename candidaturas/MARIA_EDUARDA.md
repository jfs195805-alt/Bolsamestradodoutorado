...
2. Dossiê do orientador pretendido (Fase 3) → projeto alinhado (Fase 4).
3. Auditoria adversarial → humanização → formatação DOCX (scripts prontos).
4. Materiais de defesa + dashboard de impacto (o painel dela já é o artefato).

## 📋 Preencher (placeholders reais)
- Orientador(a) do TCC; 2ª graduação (Educação Especial?) — confirmar se real.
- Cursos/certificações reais (Python, Power BI, SQL…).
- Experiência profissional com dados (se houver).
- Link público do painel quando publicar.
- Nível de inglês.

> Regra Zero: nada inventado. O que não for real vira `[PREENCHER]` e só o candidato completa.
